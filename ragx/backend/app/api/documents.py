...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        existing_doc = DocumentRegistryService.get_document_by_name(filename, status_filter=None)
        ...
        vector_db.delete_document_chunks_by_id(doc_id)

        # Index chunks in ChromaDB tagged with document_id
...
@router.get("")
async def list_documents(status: str = Query(None, description="Filter documents by status: ACTIVE or DELETED")):
    documents = DocumentRegistryService.get_all_documents(status_filter=status)
    return {
...
async def view_document(document_id: str):
    ...
    """
    doc = DocumentRegistryService.get_document_by_id(document_id)
    if not doc:
        # Fallback search by filename if legacy call
        doc = DocumentRegistryService.get_document_by_name(document_id, status_filter=None)
...
async def soft_delete_document(document_id: str):
    ...
    doc = DocumentRegistryService.get_document_by_id(document_id)
    if not doc:
        doc = DocumentRegistryService.get_document_by_name(document_id, status_filter="ACTIVE")
    ...
    deleted_chunks = vector_db.delete_document_chunks_by_id(doc_id)

    # 3. Update registry state
    updated = DocumentRegistryService.soft_delete_document(doc_id)
...
async def restore_document(document_id: str):
    ...
    doc = DocumentRegistryService.get_document_by_id(document_id)
    if not doc:
        doc = DocumentRegistryService.get_document_by_name(document_id, status_filter="DELETED")
    ...
    DocumentRegistryService.restore_document(doc_id)
...
async def permanently_delete_document(document_id: str):
    """
    ...
    removed_record = DocumentRegistryService.permanently_delete_document(doc_id)
